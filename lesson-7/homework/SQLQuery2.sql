...
/*Task 3*/
SELECT COUNT(CustomerID) as Amount FROM Customers
...
/*Task 7*/
SELECT DepartmentName,COUNT(EmployeeID) as Amount FROM Employees GROUP BY DepartmentName
...
/*Task 10*/
SELECT DepartmentName from Employees GROUP BY DepartmentName HAVING SUM(EmployeeID) > 5
...
SELECT COUNT(EmployeeID) as Amount FROM Employees WHERE DepartmentName = 'HR'

/*Task 13*/
SELECT DepartmentName, MIN(Salary) as min_sal, MAX(Salary) as max_sal FROM Employees GROUP BY DepartmentName

/*Task 14*/
SELECT DepartmentName,AVG(Salary)as avg_sal FROM Employees GROUP BY DepartmentName

/*Task 15*/
SELECT DepartmentName,AVG(Salary) as avg_sal, COUNT(*) as Employee_amount FROM Employees GROUP BY DepartmentName

/*Task 16*/
SELECT Category, Avg(Price) FROM Products GROUP BY Category HAVING Avg(Price) > 400

/*Task 17*/
SELECT year(SaleDate) as years, SUM(SaleAmount) as total_sales FROM Sales GROUP BY year(SaleDate)
...
/*Task 22*/
SELECT DepartmentName, SUM(Salary) as total_sal, AVG(Salary) as avg_sal FROM Employees GROUP BY DepartmentName HAVING AVG(Salary) > 65000

/*Task 23*/
SELECT CustomerID, SUM(TotalAmount) FROM Orders GROUP BY CustomerID HAVING SUM(TotalAmount) > 50

/*Task 24*/
SELECT year(OrderDate) as year ,month(OrderDate) as year, SUM(TotalAmount) as total_amount, COUNT(DISTINCT ProductID) as Amount FROM Orders GROUP BY YEAR(OrderDate), MONTH(OrderDate) HAVING COUNT(DISTINCT ProductID) >= 2

/*Task 25*/
SELECT YEAR(OrderDate) as year_ ,MIN(Quantity) as min_, MAX(Quantity) as max_ FROM Orders GROUP BY YEAR(OrderDate)